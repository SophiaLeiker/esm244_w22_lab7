...
Use `View(ca_counties)` to check out what it contains. Let's simplify it by only keeping two attributes: NAME (county name) and ALAND (land area), then renaming those to `county_name` and `land_area`. (The geomerty is always going to stick with the rows) 
...
ggplot(data = ca_subset_sf) +
  ...
  theme_void() + #this gets rid of the background
...
- We didn't put x and y in the aes --> since it is in sf, it automatically looks for the geom column in order to spit out the output
...
```{r}
#this is pulling in data that is in the form of points 
...
```{r}
### if you know the EPSG code (this is transforming the data frame using digits for new cordinate reference system)
sesbania_3857_sf <- st_transform(sesbania_sf, 3857)
### if you don't know the EPSG code (this is transforming the sesbania_sf crs to match the the ca_coutries_sf dataset)
...
ggplot() +
  ...
  geom_sf(data = sesbania_3857_sf, size = 1, color = "red") #this is printing the points!
```
...
sesb_counts_sf <- ca_sesb_sf %>% 
  ...
  summarize(n_records = sum(!is.na(id))) #this is smashing down the data set so it counts up how many rows are present and do not have NAs, if they all have NAs those will be ranked as 0, if there is 5 rows with actual info then that row will be defined with an n_record as 5
...
tm_shape(ca_subset_sf) +
  tm_fill("land_area", palette = "BuGn") + #filling based on landarea 
